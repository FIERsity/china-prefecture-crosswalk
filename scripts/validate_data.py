...
def main() -> None:
    ...
    require(len(xian_counties) == 5, "1983 Xi'an county transfer must contain five county rows")
    require({row["county_name_at_event"] for row in xian_counties} == {"临潼县", "蓝田县", "户县", "周至县", "高陵县"}, "1983 Xi'an county names were not normalized individually")
    require(not any("二县" in row["county_name_at_event"] or "三县" in row["county_name_at_event"] for row in xian_counties), "count phrase leaked into county name")
    require({row["to_entity_id"] for row in xian_counties} == {"CNUR-000289"}, "1983 Xi'an county transfer target mismatch")
