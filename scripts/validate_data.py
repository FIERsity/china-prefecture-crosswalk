...
def main() -> None:
    ...
    require(len(historical_events) == 104, "historical normalized event count changed")
    require(sum(row["normalization_status"].startswith("accepted_") for row in historical_events) == 104, "accepted historical event count changed")
    ...
    require(len(unified_events) == 167, "unified event count changed")
    ...
    require(all(row["source_url"].startswith("https://zh.wikipedia.org/wiki/") or row["source_url"].startswith("https://zh.wikisource.org/wiki/") or row["source_url"].startswith("http://www.jieyang.gov.cn/") for row in unified_events), "unified event source missing")
    ...
    require(sum(row["review_status"].startswith("accepted_") for row in unified_events) == 167, "accepted unified event count changed")
    ...
    external_map = json.loads((map_manifest_path.parent / "external" / "external_current.geojson").read_text(encoding="utf-8"))
    require({feature["properties"]["display_name_zh"] for feature in external_map["features"]} == {"香港特别行政区", "澳门特别行政区", "台湾省"}, "external Hong Kong/Macau/Taiwan display layer is incomplete")
    require(all(feature["properties"]["external_only"] and feature["properties"]["prefecture_name"] == "" and feature["properties"]["entity_id"] == "" for feature in external_map["features"]), "external layer must not be assigned CNUR or prefecture parents")
