...
def main() -> None:
    ...
    require(len(historical_events) == 94, "historical normalized event count changed")
    require(sum(row["normalization_status"].startswith("accepted_") for row in historical_events) == 94, "accepted historical event count changed")
    ...
    require(len(unified_events) == 157, "unified event count changed")
    ...
    require(sum(row["review_status"].startswith("accepted_") for row in unified_events) == 157, "accepted unified event count changed")
