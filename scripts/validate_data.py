...
def main() -> None:
    ...
    require(sum(row["normalization_status"].startswith("accepted_") for row in historical_events) == 76, "accepted historical event count changed")
    ...
    require(sum(row["review_status"].startswith("accepted_") for row in unified_events) == 139, "accepted unified event count changed")
    require(sum(row["review_status"] == "review_required" for row in unified_events) == 5, "unresolved unified event count changed")
