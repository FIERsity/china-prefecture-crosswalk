...
当前网站外部展示层按层级处理港澳台：省级使用三份当前背景面；地级层追加台湾 geoBoundaries ADM1（22 个外部地级对应区）；县级层追加台湾 ADM2（368 个）、香港 18 区和澳门 1 个未拆分外部展示区。县级只显示所选省份或区域的细节；切换到地级、省级时加载全国完整图层，并继续聚焦刚才的省级目标。显示名称统一以台湾省、香港特别行政区、澳门特别行政区为省级名称；香港、澳门不设置地级父节点，三者都不分配 CNUR，也不参与历史面板。
